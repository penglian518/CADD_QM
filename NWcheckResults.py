...
#
# @purpose
#   to check the output for NWChem
#
# @Author Peng Lian
...
def checkfail(file, silence=False):
    ...
    if n < 0:
        ...
        while n < olen-1:
            if 'convergence criteria' in o[n]:
                ...
                reason = 'conv'
                break
            elif ' NEB calculation not converged' in o[n]:
                warn = color.prp + file + ' NEB calculation failed to converge.' + color.end
                if not silence:
                    print warn
                fail = 'y'
                reason = 'ENBconv'
    ...
    NormalTerm = checkNormalTermination(file)
    print NormalTerm, fail, reason, olen, warn

    if NormalTerm == True and fail == 'n':
        pass
    else:
        fail = 'y'
        if reason in ['empt', '']:
            reason = 'UNK. Please check the NWChem output file %s' % file
...
def checkNormalTermination(file):
    ...
    for line in fcon:
        if line.find('Please cite the following reference when publishing') > 0:
            flag = True
...
def getfreq(file):
    ...
    fcon = open(file).readlines()
    freqlists = [i.strip().split()[1:] for i in fcon if i.startswith(' P.Frequency')]

    for i in freqlists:
        for j in  i:
            freq = round(float(j), 2)
            freqs.append(freq)
            if freq < 0:
                ifreqs.append(freq)
...
def getDFTEnergy(file, theory='DFT'):
    try:
        fcon = open(file).readlines()
    except IOError:
        print 'File %s is not exist.' % file
        return False
    if theory in ['dft', 'DFT']:
        dft_lines = [i.strip() for i in fcon if i.startswith('         Total DFT energy')]
    elif theory in ['sodft', 'SODFT']:
        dft_lines = [i.strip() for i in fcon if i.startswith('      Total SO-DFT energy')]

    DFT_En = [float(i.split('=')[1].split()[0]) for i in dft_lines]
